...
class SVMCustom:
    ...
    def fitModel(self, X, y):
        self.model = SVC(kernel="linear", C=1.0, gamma="scale", probability=True, random_state=100)
        self.model.fit(X, y)
    ...
    def predict(self, X, cuttoff = 0.8):
        y_pred_proba = self.model.predict_proba(X)[:,1]
        y_pred = [1 if proba > cuttoff else 0 for proba in y_pred_proba]
        return y_pred
...
if __name__ == "__main__":
    ...
    print("Starting fitPreprocess...")
    svm_x_train = svm.fitPreproccess(X_train)
    print("Finished fitPreprocess. Shape of training data:", svm_x_train.shape)

    print("Starting model fit...")
    svm.fitModel(svm_x_train, y_train)
    print("Finished model fit.")

    print("Starting preprocessTest...")
    svm_x_test = svm.preprocessTest(X_test)
    print("Finished preprocessTest. Shape of test data:", svm_x_test.shape)
    ...
    confusion_matrix = metrics.confusion_matrix(y_test_true, svm_pred)
